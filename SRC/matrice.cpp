...
Matrice::Matrice(const Parametre &param, std::vector<double> a, std::vector<double> b, std::vector<double> c)
{
    double kappaSta = param.kappa;
    double hSta = param.Lx / param.M;
    double hcSta = param.hc;
    ...
    // Stocker les paramètres utiles pour les méthodes membres
    this->MSta = MSta;
    this->hcSta = hcSta;
    this->pSta = pSta;
    this->TeSta = TeSta;
    this->SSta = SSta;
    this->Phi_pSta = Phi_pSta;
    this->hSta = hSta;
}

// matriceTridiagonale ne change pas
std::vector<std::vector<double>> Matrice::matriceTridiagonale() const
{
    std::vector<std::vector<double>> matrice(MSta, std::vector<double>(MSta, 0));
    for (int i = 0; i < MSta; i++)
...
// secondMembre utilise les membres stockés dans l'objet
std::vector<double> Matrice::secondMembre() const
{
    std::vector<double> F;
    F.push_back(Phi_pSta);
