...
private:
    ...
    // Membres pour les méthodes instationnaires
    double kappaInsta, hInsta, hcInsta, SInsta, pInsta, TeInsta, Phi_pInsta, rhoIsnta, CpIsnta, TF, deltaT;
    int MInsta, NInsta, MxInsta, MyInsta, MzInsta;

public:
    // Constructeur principal : passage de Parametre
    ResolutionInst(const Parametre &param);

    // getters et setters
    double getDiagInstL(int i) const { return diagInstL[i]; }
    double getSurDiagInstU(int i) const { return surDiagInstU[i]; }
    int getTaille() const { return MInsta; }
