...
private:
    ...
    int MSta;
    double hcSta, pSta, TeSta, SSta, Phi_pSta, hSta;

public:
    // Nouveau constructeur principal : passage de Parametre
    Matrice(const Parametre &param, std::vector<double> a, std::vector<double> b, std::vector<double> c);

    // Autre constructeur (si besoin)
    Matrice(int taille) : taille(taille) {}
